...
export default async function ClientesPage({
  ...
  let query = supabase
    .from('clientes')
    .select('*, empresa:empresas(id, razon_social)', { count: 'exact' })
  ...
  const { data: clientes, count } = await query.limit(50)
  const clientesList = (clientes ?? []) as ClienteConEmpresa[]
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm font-outfit text-gray-500 mt-0.5">{count ?? 0} clientes registrados</p>
        </div>
      ...
      <form action="/admin/clientes" method="GET" className="flex gap-2">
        ...
        />
        <button type="submit" className="px-4 py-2 bg-viflomax-verde text-white text-sm font-outfit rounded-lg hover:bg-viflomax-verde-claro transition-colors">
          Buscar
        </button>
        {q && (
          <Link href="/admin/clientes" className="px-4 py-2 border border-gray-300 text-gray-700 text-sm font-outfit rounded-lg hover:bg-gray-50 transition-colors">
            Limpiar
          </Link>
        )}
      </form>
